...
pam.data$SiteTemp<-paste(pam.data$reef.site,pam.data$temperature, sep = "_")
pam.data$SiteTempRep<-paste(pam.data$reef.site,pam.data$temperature,pam.data$replicate.group, sep = "_")
...
pam.cbass.model <- lmer(yield ~ reef.site * temperature + (1|replicate.group), data = cbass)
anova(pam.cbass.model)
...
levels(classic$temperature)

# Mixed Effects model - random effect of replicate.group
pam.classic.model <- lmer(yield ~ reef.site * temperature + (1 | replicate.group), data = classic)
